        ...
        for(int x = box.minX(); x <= box.maxX(); ++x) {
            for(int z = box.minZ(); z <= box.maxZ(); ++z) {
                ...
                for(BridgeConfig.Extension extension : config.get().value().extensions()) {
                    if (state.is(extension.blocks()) && blockBox.isInside(pos) && piecesContainer.isInsidePiece(pos)) {
                        for(int y = baseY - 1; y > minY; --y) {
                            pos.setY(y);
                            if (!(world.isEmptyBlock(pos) || world.getBlockState(pos).liquid() || world.getBlockState(pos).is(BlockTags.DIRT))) {
                                break;
                            }

                            world.setBlock(pos, extension.extendedState().getState(random, pos), 3);
                        }
                    }
                }
            }
